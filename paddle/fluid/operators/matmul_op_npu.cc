 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (x->dims().size() == 2) {
      ...
      const auto& runner =
          NpuOpRunner("BatchMatMulV2", {*x, *y}, {*out},
    ...
    if (x->dims().size() == 2) {
      if (transpose_y) {
        if (dx) {
          ...
          const auto& runner_dx =
              NpuOpRunner("MatMulV2", {*dout, *y}, {*dx},
        ...
        if (dy) {
          ...
          const auto& runner_dy =
              NpuOpRunner("MatMulV2", {*dout, *x}, {*dy},
        ...
        if (dy) {
          dy->mutable_data<T>(ctx.GetPlace());
          const auto& runner_dy =
              NpuOpRunner("MatMulV2", {*x, *dout}, {*dy},
                          {{"transpose_x1", true}, {"transpose_x2", false}});
      ...
      if (transpose_y) {
        if (dx) {
          dx->mutable_data<T>(ctx.GetPlace());
          const auto& runner_dx =
              NpuOpRunner("BatchMatMulV2", {*dout, *y}, {*dx},
        ...
        if (dy) {
          dy->mutable_data<T>(ctx.GetPlace());
          const auto& runner_dy =
              NpuOpRunner("BatchMatMulV2", {*dout, *x}, {*dy},
        ...
        if (dy) {
          dy->mutable_data<T>(ctx.GetPlace());
          const auto& runner_dy =
              NpuOpRunner("BatchMatMulV2", {*x, *dout}, {*dy},
                          {{"adj_x1", true}, {"adj_x2", false}});
          runner_dy.Run(stream);
        }
      }
