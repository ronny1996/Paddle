...
class TestGaussianRandomOp(OpTest):
    ...
    def setUp(self):
        ...
        self.outputs = {'Out': np.zeros(self.shape, dtype='float32')}
    ...
    def verify_output(self, outs):
        hist, _ = np.histogram(outs[0], range=(-5, 5))
        hist = hist.astype("float32")
        hist /= float(outs[0].size)
        data = np.random.normal(size=self.shape, loc=self.mean, scale=self.std)
        hist2, _ = np.histogram(data, range=(-5, 5))
